import Vue from 'vue'
import VueRouter from 'vue-router'
import Layout from '@/layout'
import store from '@/store'

Vue.use(VueRouter)

// 创建基本路由 (移除通配符路由)
const routes = [
  {
    path: '/login',
    name: 'Login',
    component: () => import('@/views/Login.vue'),
    hidden: true
  },
  {
    path: '/404',
    component: () => import('@/views/404.vue'),
    name: 'NotFound',
    hidden: true
  },
  {
    path: '/route-loading',
    component: () => import('@/views/RouteLoading.vue'),
    name: 'RouteLoading',
    hidden: true
  }
]

const router = new VueRouter({
  mode: 'history',
  base: process.env.BASE_URL,
  routes
})

// 兼容的检查路由是否存在的方法
function routeExists(name) {
  return router.getRoutes().some(route => route.name === name)
}

// 修复后的动态路由添加方法
export function addDynamicRoutes(menuTree) {
  console.log('开始添加动态路由，菜单树:', menuTree)

  // 兼容方法替换：检查通配符路由是否存在
  if (routeExists('Wildcard')) {
    try {
      // 尝试移除通配符路由
      if (router.removeRoute) {
        router.removeRoute('Wildcard')
      } else {
        // 对于旧版本Vue Router，使用更复杂的方式移除路由
        const routes = router.options.routes.filter(r => r.name !== 'Wildcard')
        router.matcher = new VueRouter({ routes }).matcher
      }
    } catch (e) {
      console.warn('移除通配符路由失败', e)
    }
  }

  // 1. 创建新的布局路由 - 关键修改：添加唯一标识
  const layoutRoute = {
    path: '/',
    component: Layout,
    redirect: '/dashboard',
    name: 'Layout',
    meta: {
      isRootLayout: true, // 添加标识用于面包屑过滤
      hideInBreadcrumb: true // 不在面包屑中显示
    },
    children: [
      {
        path: '/dashboard',
        component: () => import('@/views/Dashboard.vue'),
        name: 'Dashboard',
        meta: { title: '控制面板', icon: 'dashboard' }
      }
    ]
  }

  // 2. 递归添加菜单路由 - 关键修改：添加路由隐藏标识
  const addRoutes = (menuItems, parentRoute = null, level = 0) => {
    menuItems.forEach(item => {
      // 处理有子菜单的菜单项
      if (item.children && item.children.length) {
        const route = {
          path: item.path || `/menu-${item.id}`,
          component: level === 0 ? Layout : () => import('@/views/NestedContainer.vue'),
          name: `Menu_${item.id}`,
          meta: { 
            title: item.title, 
            icon: item.icon,
            hideInBreadcrumb: level > 0 // 嵌套路由在面包屑中隐藏
          },
          redirect: item.children[0].path,
          children: []
        }

        // 添加到父路由或根布局
        if (parentRoute) {
          parentRoute.children.push(route)
        } else {
          layoutRoute.children.push(route)
        }

        // 递归处理子菜单
        addRoutes(item.children, route, level + 1)
      }
      // 处理叶子节点
      else if (item.path) {
        const leafRoute = {
          path: item.path,
          component: () => import(`@/views/${item.component || 'EmptyPage'}.vue`),
          name: `Page_${item.id}`,
          meta: { 
            title: item.title, 
            icon: item.icon,
            isLeaf: true // 标记为叶子节点
          }
        }

        if (parentRoute) {
          parentRoute.children.push(leafRoute)
        } else {
          layoutRoute.children.push(leafRoute)
        }
      }
    })
  }

  // 3. 添加菜单树到布局路由
  addRoutes(menuTree)

  // 4. 移除旧布局路由（如果存在）
  if (routeExists('Layout')) {
    try {
      if (router.removeRoute) {
        router.removeRoute('Layout')
      } else {
        // 旧版本处理
        const routes = router.options.routes.filter(r => r.name !== 'Layout')
        router.matcher = new VueRouter({ routes }).matcher
      }
    } catch (e) {
      console.warn('移除旧布局路由失败', e)
    }
  }

  // 5. 添加新的布局路由
  console.log('添加新布局路由:', layoutRoute);
  router.addRoute(layoutRoute)

  // 6. 最后添加通配符路由
  router.addRoute({
    path: '*',
    redirect: '/404',
    name: 'Wildcard',
    hidden: true
  })

  console.log('动态路由添加完成', router.getRoutes())
}

// 改进的路由守卫 - 添加调试信息
router.beforeEach(async (to, from, next) => {
  console.log(`路由跳转: ${from.path} → ${to.path}`)
  console.log('路由匹配记录:', to.matched.map(r => r.path))
  console.log('路由名称:', to.matched.map(r => r.name))

  // 无需认证的白名单
  const noAuthPages = ['/login', '/404', '/route-loading']

  if (noAuthPages.includes(to.path)) {
    return next()
  }

  // 1. 检查认证令牌
  const token = localStorage.getItem('token')
  if (!token) {
    return next('/login')
  }

  // 2. 检查动态路由是否已加载
  // 检查菜单树是否已加载
  if (store.state.menuTree.length === 0) {
    try {
      // 优先从localStorage加载
      const savedMenuTree = JSON.parse(localStorage.getItem('menuTree') || '[]');
      console.log('从localStorage读取的menuTree:', savedMenuTree);
      if (savedMenuTree.length) {
        store.commit('SET_MENU_TREE', savedMenuTree);
        addDynamicRoutes(savedMenuTree);
        return next(to.path); // 重新触发当前导航
      }

      // 无缓存则请求API
      await store.dispatch('fetchUserInfo');
      const freshMenuTree = store.state.menuTree;
      addDynamicRoutes(freshMenuTree);

    } catch (error) {
      console.error('路由加载失败:', error);
      localStorage.removeItem('token');
      return next('/login');
    }
  }

  // 3. 确保路由存在
  if (to.matched.length === 0) {
    console.warn(`路由未匹配: ${to.path}`)
    return next('/404')
  }

  next()
})

export default router
